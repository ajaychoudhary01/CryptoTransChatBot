...
(function () {
  ...
  exports.audioControl = function (options) {
     ...
     * @param {flag} The flag to use in the export.
     * @throws {Error} If audio is not supported.
     */
    var exportWAV = function (flag, callback) {
      audioSupported = audioSupported !== false;
      if (!audioSupported) {
        throw new Error(UNSUPPORTED);
      }
      if (!(callback && typeof callback === 'function')) {
        ...
        recorder.exportWAV(callback, 16000, flag);
    };
...
(function() {
  ...
  var Listening = function(state) {
    ...
    this.advanceConversation = function() {
        audioControl.exportWAV(false, function(blob) {
            state.audioInput16 = blob;
            addAudioPlayer(blob);
            audioControl.exportWAV(false, function (blob1) {
                state.audioInput = blob1;
  ...
  function getIds(state) {
    ...
    function verifyIdentity(ids, state) {
        ...
        request.send(state.audioInput16);
    }
...
module.exports = function (fn, options) {
 ...
 (function () {
  ...
  var recorder = function (source, silenceDetectionConfig) {
     ...
     * @param {sampleRate} The sample rate to use in the export.
     */
      var exportWAV = function (callback, sampleRate, flag) {
...
module.exports = function (self) {
  ...
  self.addEventListener('message', function (e) {
    switch (e.data.command) {
      ...
      case 'record':
        ...
        break;
        case 'export': {
         exportBuffer(e.data.sampleRate, e.data.flag);
         break;
        }
      case 'clear':
        clear();
    ...
    function init(config) {
        console.log('recordSampleRate' + config.sampleRate);
  ...
  function exportBuffer(exportSampleRate, flag) {
      var mergedBuffers = mergeBuffers(recBuffer, recLength);
      var downsampledBuffer = downsampleBuffer(mergedBuffers, exportSampleRate, flag);
  ...
  function clear() {
    ...
    function downsampleBuffer(buffer, exportSampleRate, flag) {
        if (flag) {
        ...
        if (exportSampleRate === recordSampleRate) {
            console.log('I am called ' + exportSampleRate + ' ' + recordSampleRate + ' ' + flag);
      return buffer;
        }
        console.log('I am called also ' + exportSampleRate + ' ' + recordSampleRate + ' ' + flag);
